...
#     model = nn.DataParallel(model)
    ...
    #         torch.save(model.state_dict(), 'C:/data/computer_vision2/modelcheckpoint/EfficientNetB0-fold{}.pt'.format(fold))
...
with torch.no_grad():
    for fold in range(1):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('C:/data/computer_vision2/modelcheckpoint/EfficientNetB0-fold{}.pt'.format(fold)))
        model.eval()
...
submission.to_csv('C:/data/computer_vision2/EfficientNetB0-fold0.csv', index=False)