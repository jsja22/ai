...
def model_layers():
    ...
    return x
def build_model(drop=0.5,optimizer='adam',activation='relu',lr=0.001,nodes=64,kernel_size = (2, 2)):
    Inputs = Input(shape=(28,28,1),name='input')
    x = Conv2D(512, kernel_size=(2,2),activation='relu',padding='same',name='hiddne1')(Inputs)
    x = MaxPooling2D(pool_size=2)(x)
    x = Dropout(drop)(x)
    x = Conv2D(256, kernel_size=(2,2),activation='relu',padding='same',name='hiddne2')(x)
    x = MaxPooling2D(pool_size=2)(x)
    x = Dropout(drop)(x)
    x = Conv2D(128, kernel_size=(2,2),activation='relu',padding='same',name='hiddne3')(x)
    x = MaxPooling2D(pool_size=2)(x)
    x = Dropout(drop)(x)

    x = Flatten()(x)
    x = Dense(nodes, activation='relu',name='hiddne4')(x)
    x = Dense(nodes, activation='relu',name='hiddne5')(x)
    x = Dense(nodes, activation='relu',name='hiddne6')(x)
...
def create_hyperparameters():
    ...
    kernel_size = [2, 3, 4]
    epochs = [100, 200, 300]
    return {"batch_size" : batchs, "optimizer":optimizers, "drop":dropout, "activation":activation, "lr":lr, "nodes":nodes1,"nodes":nodes2,"kernel_size": kernel_size, "epochs": epochs}
