...
#l1. 
w1 = tf.compat.v1.get_variable("w1",shape=[3,3,1,32]) #3,3,1,32  3,3 => 커널사이즈 1 => 채널 32=> filtters(output)
...
w2 = tf.compat.v1.get_variable("w2",shape=[3,3,32,64]) #3,3,1,32  3,3 => 커널사이즈 1 => 채널 32=> filtters(output)
...
w3 = tf.compat.v1.get_variable("w3",shape=[3,3,64,128]) #3,3,1,32  3,3 => 커널사이즈 1 => 채널 32=> filtters(output)
L3 = tf.nn.conv2d(L2,w3,strides=[1,1,1,1], padding = 'SAME') #(None,7,7,128) 
L3 = tf.nn.selu(L3)
L3 = tf.nn.max_pool(L3, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME')
print(L3)

w4 = tf.compat.v1.get_variable("w4",shape=[3,3,128,64]) #3,3,1,32  3,3 => 커널사이즈 1 => 채널 32=> filtters(output)
...
#L5.
w5 = tf.compat.v1.get_variable("W5", shape=[2*2*64,64],initializer=tf.compat.v1.initializers.variance_scaling())
...
#L6.
w6 = tf.compat.v1.get_variable("W6", shape=[64,32],initializer=tf.compat.v1.initializers.variance_scaling())
...
#L7.
w7 = tf.compat.v1.get_variable("W7", shape=[32,10],initializer=tf.compat.v1.initializers.variance_scaling())
