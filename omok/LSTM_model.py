...
p_path = os.getcwd()
print(p_path)
base_path = os.path.join('freestyle3', '*/*.npz')
...
print(x_data.shape) #(503512, 15, 15, 1)
print(y_data.shape) #(503512, 225)

seed =2048
x_train, x_val, y_train, y_val = train_test_split(x_data, y_data, test_size=0.2, random_state=seed)

model = Sequential()
model.add(LSTM(128, activation='relu', input_shape=(225,1)))
...
mc = ModelCheckpoint('C:/data/omok/modelcheckpoint/last_1_%s.h5' % (start_time), monitor='val_acc', verbose=1, save_best_only=True, mode='auto')
...
y_pred = model.predict(np.expand_dims(x_val[0], axis=0)).squeeze() #squeeze는 차원 중 사이즈가 1인 것을 찾아 스칼라값으로 바꿔 해당 차원을 제거
