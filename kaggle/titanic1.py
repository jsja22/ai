...
train_df = pd.read_csv('C:/data/kaggle/train.csv')
test_df = pd.read_csv('C:/data/kaggle/test.csv')
submission = pd.read_csv('C:/data/kaggle/sample_submission.csv')

#pclass -> 좌석 등급 1st,2st,3st
#sex ->성별
#Age -> 나이
#sibsp -> 타이타닉호에 탑승한 형제자매/배우자 수
#parch -> 타이타닉호에 탑승한 부모/아이들의 수
...
pseudo_labels = pd.read_csv("C:/data/kaggle/pseudo_label.csv")

#Labeled Data처럼 일일히 label을 하기보다, 이미 가지고 있는 Labeled data에 기반하여 대략적인 Label을 주는 것을 Pseudo Labelling
test_df[TARGET] = pseudo_labels[TARGET]  #test 타겟값을 pseudo_label로  라벨링함

all_df = pd.concat([train_df, test_df]).reset_index(drop=True) #reset_index(drop=True) -> 기존의 행 인덱스를 제거하고 인덱스를 데이터 열로 추가 
                                                                # drop=True 로 설정하면 인덱스 열을 보통의 자료열로 올리는 것이 아니라 그냥 버리게 된다

print(all_df.head(5))                                                      
# Age fillna with mean age for each class
all_df['Age'] = all_df['Age'].fillna(all_df['Age'].mean())  #age 칼럼 평균으로 결측값 채우기 
                                                            #특정값으로 채우고 싶으면 fill_values 사용
# Cabin, fillna with 'X' and take first letter
all_df['Cabin'] = all_df['Cabin'].fillna('X').map(lambda x: x[0].strip())

# Ticket, fillna with 'X', split string and take first split 
all_df['Ticket'] = all_df['Ticket'].fillna('X').map(lambda x:str(x).split()[0] if len(str(x).split()) > 1 else 'X')
#lamnbda 표현식내에서 if사용하면 반드시 else사용해야함 
# Fare, fillna with mean value
fare_map = all_df[['Fare', 'Pclass']].dropna().groupby('Pclass').median().to_dict()  #median은 열 축의 최소값부터 최대값 사이중 중앙값을 찾아줌 이를 딕셔너리형태로 변환
all_df['Fare'] = all_df['Fare'].fillna(all_df['Pclass'].map(fare_map['Fare']))
all_df['Fare'] = np.log1p(all_df['Fare'])
#np.log1p-> '-inf' -> '0'으로 바뀌게 됨
...
submission['submit_2'] = pd.read_csv("C:/data/kaggle/dae.csv")[TARGET]
...
submission[['PassengerId', TARGET]].to_csv("C:/data/kaggle/csv/sub8.csv", index = False)
