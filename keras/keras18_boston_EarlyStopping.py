...
early_stopping = EarlyStopping(monitor='loss', patience=10,mode='auto')  #loss의 최소값 loss값까지 더 떨어지지 않는 값을 10번참겠다는 뜻


#model.fit(x_train,y_train,epochs=100, batch_size=8, validation_split=0.2,verbose=0)
model.fit(x_train, y_train, epochs=500, batch_size=8, validation_data =(x_val,y_val),cllbacks=[early_stopping])
