...
dataset_data1 = dataset_data1.reshape(dataset_data1.shape[0],dataset_data1.shape[1]*dataset_data1.shape[2])
...
np.save('C:/data/npy/samsung_x_pred.npy', arr=x_pred)


# 모델

model = Sequential()
model.add(Conv1D(filters = 800, kernel_size = 2, strides=1, padding = 'same', input_shape = (x_train.shape[1],x_train.shape[2]), activation='relu'))
