...
scaler = QuantileTransformer() #1000개의 분위수 #평균값이 아닌 분위수로 한다 따라서 이상치에대해 자유로워짐(이상치 제어에 효과가 있다.)
#디폴트 : 균등분포
scaler = QuantileTransformer(output_distribution='normal') #1000개의 분위수 #평균값이 아닌 분위수로 한다 따라서 이상치에대해 자유로워짐(이상치 제어에 효과가 있다.)
#정규분포
scaler.fit(x)
