...
from sklearn.datasets import load_iris  ,load_breast_cancer,load_wine #다중 분류모델
...
#2. model
pipe = Pipeline([("scaler",MinMaxScaler()),('RF',RandomForestClassifier())]) #svc 란 모델과 민맥스란 전처리 하나하나 합친것
#model = make_pipeline(MinMaxScaler(),SVC())
#pipe = make_pipeline(StandardScaler(),SVC())

model = RandomizedSearchCV(pipe,parameters, cv=5)
score = cross_val_score(model,x_train,y_train,cv=5)  #score는 5번씩 5개 총 25번 돌아간다.
