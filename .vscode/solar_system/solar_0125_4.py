...
for i in range(48):
    ...
    for j in quantiles:
        ...
        model = Conv1dml()
        filepath_cp = f'C:/data/modelcheckpoint/solar_checkpoint_0125_5{i:2d}_y1seq_{j:.1f}.hdf5'
    ...
    for j in quantiles:
        ...
        model = Conv1dml()
        filepath_cp = f'C:/data/modelcheckpoint/solar_checkpoint_0125_5{i:2d}_y2seq_{j:.1f}.hdf5'
        cp = ModelCheckpoint(filepath_cp,save_best_only=True,monitor = 'val_loss')
        model.compile(loss = lambda y_true,y_pred: quantile_loss(j,y_true,y_pred), optimizer = 'adam', metrics = [lambda y,y_pred: quantile_loss(j,y,y_pred)])
