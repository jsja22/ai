...
for i in range(48):
    ...
    for j in quantiles:
        ...
        model = Conv1dmodel()
        filepath_cp = f'C:/data/modelcheckpoint/solar_checkpoint_0125_4{i:2d}_day1_{j:.1f}.hdf5'
    ...
    for j in quantiles:
        ...
        model = Conv1dmodel()
        filepath_cp = f'C:/data/modelcheckpoint/solar_checkpoint_0125_4{i:2d}_day2_{j:.1f}.hdf5'
        cp = ModelCheckpoint(filepath_cp,save_best_only=True,monitor = 'val_loss')
        model.compile(loss = lambda y_true,y_pred: quantile_loss(j,y_true,y_pred), optimizer = 'adam', metrics = [lambda y,y_pred: quantile_loss(j,y,y_pred)])
