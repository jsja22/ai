...
idg = ImageDataGenerator(rotation_range=20,height_shift_range=0.1,width_shift_range=0.1) #augmentation 인자를 지정
...
sample_datagen = ImageDataGenerator(rotation_range=20,height_shift_range=0.1, width_shift_range=0.1)
...
for train_index, valid_index in skf.split(train2,train['digit']) :
    ...
    mc = ModelCheckpoint('C:/data/computer vision/h5/Dacon_computer_vision_0203_4.h5',save_best_only=True, verbose=1)
    ...
    # predict
    model.load_weights('C:/data/computer vision/h5/Dacon_computer_vision_0203_4.h5')
...
sub.to_csv('C:/data/computer vision/csv/Dacon_computer_vision_0203_4.csv',index=False)


##rotation =20, shear_range =0.2  ImageDataGenerator 파라미터 이것저것 바꿔봤지만 accuracy는 
# height_shift_range=0.1, width_shift_range=0.1만 사용한게 좋았음!